...
Next, install and start minikube to run the Kubernetes cluster locally and set it
up within a Docker run-time environment.

Note that this step assumes a Linux OS, an AMD64 CPU, and one or more
Nvidia GPUs. The commands will be different if you have a different
configuration.
#+begin_src shell
