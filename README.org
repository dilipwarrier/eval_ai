...
#+begin_src shell
  minikube start \
    ...
    --memory=16384 \
    --cpus=4
  minikube image load ghcr.io/llm-d/llm-d-cuda:v0.5.1-rc.4
#+end_src
...
Apply the overall helmfile now to load the inference engine.
#+begin_src shell
...
RAM and get ready to serve prompt requests. If all goes well, you
should see a running Status with the following command.
#+begin_src shell
   kubectl describe pod ms-inference -n llm-d | grep Status:
#+end_src
...
#+begin_src shell
  ...
  # Destroy resources
  INFERENCE_GUIDE_PATH="llm-d/guides/inference-scheduling"
  helmfile -f "$INFERENCE_GUIDE_PATH/helmfile.yaml.gotmpl" destroy -n llm-d
...
#+begin_src shell
  ...
  # Load the llm-d image from the host Docker to Minikube's docker node
  minikube image load ghcr.io/llm-d/llm-d-cuda:v0.5.1-rc.4
