...
#+begin_src shell
  ...
  # Store the Process ID (PID) so you can stop it later
  export TUNNEL_PID=$!
  echo "Tunnel established at http://localhost:8080 (PID: $TUNNEL_PID)"
#+end_src
...
When you're done with your experiments, run the following to stop the
port forwarding.
#+begin_src shell
  kill $TUNNEL_PID
#+end_src
