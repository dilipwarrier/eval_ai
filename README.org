...
#+begin_src shell
  ...
  # 1. Run the script to install all dependencies
...
Next, install minikube to run the Kubernetes cluster locally and set it
...
#+begin_src shell
  ...
  # Start a Minikube node with a Docker container and access to the GPUs
  minikube start --driver=docker --gpus all

  # Make the Nvidia GPU available as a resource to Kubernetes pods
...
Run the following commands to install the istio gateway provider.
#+begin_src shell
  # Define the gateway path variable
...
#+begin_src shell
  ...
  helmfile -f "$INFERENCE_GUIDE_PATH/helmfile.yaml.gotmpl" \
           apply -n llm-d --quiet
#+end_src
...
Run a test prompt against the completions API.
#+begin_src shell
...
You should see the output in JSON form including the prompt response
and other metadata.
